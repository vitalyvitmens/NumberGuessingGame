...
var guessCount = 1;
var resetButton; // Эта строка использует метод focus(), чтобы автоматически помещать текстовый курсор в текстовое поле <input>, как только загрузится страница. Пользователь сможет сразу набрать свою первую догадку, не нажимая поле формы.
...
function checkGuess() {
  ...
  guesses.textContent += "".concat(userGuess, " "); // Блок1: проверяет, совпадает ли предположение пользователя с randomNumber
...
function resetGame() {
  ...
  guessCount = 1; // Удаляет все пункты информации. Этот код создаёт переменную, содержащую список всех абзацев внутри <div class = "resultParas">, используя метод querySelectorAll(), затем он проходит через каждый из них, удаляя текстовое содержимое каждого из них.
  ...
  resetButton.parentNode.removeChild(resetButton); // Включает элементы формы, устанавливает фокус, делает поле доступным для следующих угадываний.
